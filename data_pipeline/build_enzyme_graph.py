...
try:
    ...
    USE_PYG = False
    print("[INFO] PyTorch Geometric not found. Will save as numpy dicts.")
...
CONTACT_DISTANCE = 8.0  # Angstroms — standard cutoff for residue contact graphs

# Rate limiting for AlphaFold API
REQUEST_DELAY = 0.1  # seconds between requests (be respectful to the server)
...
# Molecular weight of amino acids (normalized by dividing by max=204 for Trp)
MOLECULAR_WEIGHT = {
...
def one_hot_aa(aa):
    """One-hot encode an amino acid (20 standard + 1 unknown)."""
    encoding = [0] * 21
    if aa in AMINO_ACIDS:
        encoding[AMINO_ACIDS.index(aa)] = 1
    else:
        encoding[20] = 1  # unknown
    return encoding


def get_residue_features(aa_letter):
    """
    Get feature vector for a single residue.
    Returns vector of dimension 24:
        - 21: one-hot amino acid type
        - 1:  hydrophobicity
        - 1:  molecular weight (normalized)
        - 1:  charge
    """
...
def download_alphafold_structure(uniprot_id, output_dir):
    ...
    Returns the path to the PDB file, or None if download fails.
    """
    ...
    # AlphaFold DB URL — v6 as of 2025/2026
...
def pdb_to_residue_graph(pdb_path, contact_distance=8.0):
    """
    Convert a PDB file to a residue-level contact graph.
    
    Nodes: residues (amino acids)
    Edges: pairs of residues with Cα-Cα distance < contact_distance
    
    Returns:
        dict with keys:
            'node_features':  np.array (num_residues, feature_dim)
            'edge_index':     np.array (2, num_edges) — bidirectional
            'edge_features':  np.array (num_edges, 1) — distance values
    ...
    # Extract residues and their Cα coordinates
    residues = []
    ca_coords = []
    sequence = []
    ...
    if len(residues) < 5:  # too small to be useful
        return None
    ...
    return {
        ...
        'ca_coords': ca_coords,  # useful for visualization later
    }
...
def build_all_enzyme_graphs():
    ...
    # Process each target
    enzyme_graphs = {}  # uniprot_id → graph dict
    download_failed = []
    parse_failed = []
    
    print(f"\nDownloading AlphaFold structures and building contact graphs...")
    ...
    if enzyme_graphs:
        ...
        avg_density = np.mean([c / (r * (r - 1)) for c, r in 
                               zip(contact_counts, residue_counts) if r > 1])
        print(f"  Average graph density: {avg_density:.4f}")
    
    # Save as pickle
    pkl_path = os.path.join(OUTPUT_DIR, "enzyme_graphs.pkl")
    # Remove ca_coords before saving (saves space, not needed for training)
    save_graphs = {}
    for uid, g in enzyme_graphs.items():
        g_save = {k: v for k, v in g.items() if k != 'ca_coords'}
        save_graphs[uid] = g_save
    
    with open(pkl_path, 'wb') as f:
        pickle.dump(save_graphs, f, protocol=pickle.HIGHEST_PROTOCOL)
    size_mb = os.path.getsize(pkl_path) / 1e6
    print(f"\n[SAVED] {pkl_path} ({size_mb:.1f} MB)")
    
    # Save as PyTorch Geometric if available
    if USE_PYG and enzyme_graphs:
        print("\nConverting to PyTorch Geometric format...")
        pyg_graphs = {}
        for uid, g in tqdm(enzyme_graphs.items(), desc="Converting to PyG"):
            data = Data(
        ...
        pyg_path = os.path.join(OUTPUT_DIR, "enzyme_graphs_pyg.pkl")
...
def show_example():
    ...
    uniprot_id = "P24941"  # Human CDK2 — a well-known drug target kinase
    ...
    print(f"Building residue contact graph (cutoff: {CONTACT_DISTANCE} Å)...")
    ...
    if graph:
        ...
        avg_contacts = graph['num_contacts'] / graph['num_residues']
        print(f"  Avg contacts/residue: {avg_contacts:.1f}")
    else:
...
if __name__ == "__main__":
    ...
    print("╠" + "═" * 68 + "╣")
    print("║   Output: ./bindingdb_data/processed/enzyme_graphs/               ║")
